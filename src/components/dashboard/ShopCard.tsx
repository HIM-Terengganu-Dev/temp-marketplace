...
function TrendArrow({ pct }: { pct?: number }) {
    ...
    if (pct > 0.5)
        return (
            <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-emerald-400 ml-1 tabular-nums">
                <TrendingUp className="h-2.5 w-2.5" />{abs}%
            </span>
        );
    if (pct < -0.5)
        return (
            <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-red-400 ml-1 tabular-nums">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-1.5 pt-3 border-t border-border/30">
                            ...
                            {data.cpasSpend !== undefined && data.cpasSpend > 0 && (
                                <div className="pl-3 border-l-2 border-slate-700/50 space-y-1 py-1">
                                    ...
                                    <div className="flex justify-between items-center text-[10px]">
                                        <span className="text-blue-400 font-medium">Meta CPAS</span>
                            ...
                            <div className="flex justify-between items-center text-xs pt-1 border-t border-border/20">
                                <span className="text-purple-400 font-medium text-[10px]">After Tax (SST+WHT)</span>
                                <span className="font-semibold text-purple-400 tabular-nums text-sm">
                        ...
                        <div className="space-y-1.5 pt-3 border-t border-border/30">
                            ...
                            <div className="flex justify-between items-baseline">
                                ...
                                <div className="flex items-baseline gap-1">
                                    <span className={cn(
                                        ...
                                        (data.roas || 0) >= 3 ? "text-emerald-400"
                                            : (data.roas || 0) >= 2 ? "text-yellow-400"
                                            : "text-red-400"
                                    )}>
                            ...
                            <div className="flex justify-between items-baseline border-t border-border/20 pt-1">
                                <span className="text-[10px] text-purple-400 font-medium">After Tax</span>
                                <span className={cn(
                                    "text-lg font-extrabold tabular-nums tracking-tight",
                                    (data.roasAfterTax || 0) >= 3 ? "text-emerald-400"
                                        : (data.roasAfterTax || 0) >= 2 ? "text-purple-400"
                                        : "text-red-400"
                                )}>
