...
async function fetchTikTokShopMetricsSWR(
    ...
    dates.forEach(date => {
        ...
        const isRecentPast = !isToday && dateDiffDays(date, today) <= 1;
    ...
    return {
        ...
        roasBeforeTax,
        roasAfterTax,
        dataSource: loadedFromDbCount > 0 && loadedFromApiCount > 0 ? 'database+api' : loadedFromDbCount > 0 ? 'database' : 'live_api'
    };
}
...
async function run() {
    console.log('Starting full summary endpoint replication test...');
    const today = getKLToday();
    
    // Testing for today: 2026-05-26 and yesterday: 2026-05-25
    const startDate = today;
    const endDate = today;
    const prevStartDate = '2026-05-25';
    const prevEndDate = '2026-05-25';

    try {
        const shopeeShops = await getConnectedShopeeShops();
        console.log(`Found ${shopeeShops.length} Shopee shops connected.`);
        
        const shopIndices = [1, 2, 3, 4];
        const backgroundRevalidateThunks: any[] = [];

        console.log('Executing parallel fetches...');
        const [
            curResults,
            prevResults,
            shopeeCurResults,
        ...
        console.log(`TikTok Current Results: ${curResults.length}`);
        console.log(`TikTok Previous Results: ${prevResults.length}`);
        console.log(`Shopee Current Results: ${shopeeCurResults.length}`);
        console.log(`Shopee Previous Results: ${shopeePrevResults.length}`);
        
        console.log('\nFirst Shopee Current Result:');
        console.log(JSON.stringify(shopeeCurResults[0], null, 2));

        console.log('\nAll Shopee Current Results IDs:');
        shopeeCurResults.forEach(r => console.log(`  Shop ID: ${r.shopId}, Name: ${r.shopName}, GMV: RM ${r.gmv}`));

    } catch (e: any) {
        console.error('Error during full replication test:', e);
    } finally {
        await pool.end();
    }
}
